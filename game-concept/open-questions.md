# Open Questions — Creator-Owned Canon Decisions

> Things the sheets don't answer (or answer twice). Nothing here blocks the
> prototype; everything here shapes the story. Answer at leisure; the bible
> gets updated as each is resolved.

## Locked canon (already decided — recorded for protection)
...
1. **"Gustave Sevirum"** (Aria's bio) — is Sevirum a surname? Are Gustave and
   Sevirum family? Same person in old records? Coincidence?
2. **Two Word Twisters** — Malthyros (Omega, "secretly supreme") vs UNKNOWN
   Word Twister (Omega+, "true name forbidden"). Same demon at different
   intel levels / predecessor / imposter?
3. **Zephyrith v1 vs v2** — treat "Unbound Edge" as canon and fold v1's
   ability names in? (Current working assumption: yes.)
4. **Xaryth vs "Xarths"** (Vorathen's sheet spelling) — same being? (Working
   assumption: yes.)
5. Xin/X and Sevirum share violet accents — related origins, or should one
   shift hue?

## Story spine

6. **Why does Odion — heir of Helticor — fight on a human Ghost squad?**
   (Exile? Mission from Zanmer? Defection? The single biggest untold story.)
7. **What was XIIX?** Who else was in it? Is Kail Squad (Aegis) its successor?
   Why is it hidden?
...
12. Where do the "demons" come from relative to the Helticors — are "demon"
    and "Helticor" human labels for related things, or truly separate
    origins? (Working assumption from sheets: separate.)
13. The Underlying (Malthyros), the Void / First Fracture (Kortharax), and
    the Ancient Way (Queen) — three metaphysical layers. One cosmology or
    three? Rationalize eventually.
14. Present-day year is ~2477 (archive stamps). Confirm the campaign's "now."
15. How public are demons/Helticors to ordinary humans in 2477?

## Game design

16. Squad play: character switching, AI companions, co-op, or
    chapter-per-Ghost? (Implosion model = single character per mission;
    roster suggests switching. Decide before prototype milestone 2.)
17. Which Ghost is the prototype's playable? (Default: Sevirum — main
    character — but Linora's kit is the most Implosion-ready melee/ranged
    hybrid. Creator call.)
18. Terran-loyalist units as companions/escort content — yes/no?
19. Eighth Ghost (support/engineer archetype) — ever?
